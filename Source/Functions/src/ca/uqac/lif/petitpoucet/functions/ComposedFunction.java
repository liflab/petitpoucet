...
public class ComposedFunction extends Function
{
			...
			Tracer factory)
	{
		...
		else if (top instanceof NthOutput)
		{
			...
			ConcreteDesignatedObject dob = new ConcreteDesignatedObject(new_cd,
					m_outputPlaceholders[no.getIndex()]);
			TraceabilityNode tn = factory.getObjectNode(dob);
			leaves.add(tn);
			root.addChild(tn, Quality.EXACT);
		}
	...
	public void getValue(Object[] inputs, Object[] outputs)
	{
		...
		Object[] computed = evaluate();
		for (int i = 0; i < computed.length; i++)
		{
			outputs[i] = computed[i];
		}
	}
	...
	{
		super.reset();
		for (InputPlaceholder p : m_inputPlaceholders)
		{
			p.reset();
		}
		for (OutputPlaceholder p : m_outputPlaceholders)
		{
			p.reset();
		}
